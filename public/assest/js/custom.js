...
var lastScrollTop = 0;
var delta = 5;
var navbarHeight = $('.RegisterForm').outerHeight();
...
function hasScrolled() {
    ...
    if (st > lastScrollTop && st > navbarHeight) {
        // Scroll Down
        $('.RegisterForm').css('top', '-600px');
    } else {
        // Scroll Up
        if (st + $(window).height() < $(document).height()) {
            $('.RegisterForm').css('top', '20%');
        }
    }
...
let apiDataGet = async () => {
    let data = getProjectData(178)       // Change Project_ID Integer Value only Ex:1131
        .then((data) =>{ 
...
// main function to store the data and post data to api
    ...
    $('#dropAnEnquiry').on("submit", function () {
     ...
     $('#ModalForm').on("submit", function () {
       ...
       var checkbox = $('#modal_checkbox').val();
        saveLead(name, email, number, countrycode,checkbox, 'Homemeeting_form');
        return false;
    })
    $('#PopUpForm').on("submit", function () {
        ...
        var countrycode = $('#PopUp_CountryCode').val();
       var checkbox = $('#PopUp_checkbox').val();
        saveLead(name, email, number, countrycode,checkbox,'Onload_Popup_overview_homepage');
        return !1
    })
